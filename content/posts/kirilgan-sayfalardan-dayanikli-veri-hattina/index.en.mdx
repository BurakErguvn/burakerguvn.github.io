...
```mermaid
flowchart LR
  Candidate["Discovered record"] --> Check{"Does it pass the filter?"}
  Check -->|No| Skipped["skipped: preserve the link"]
  Check -->|Yes| Pending["pending: add to queue"]
  Pending --> Scraping["scraping: work started"]
  Scraping --> Done["done: complete"]
  Scraping --> Failed["failed: retryable"]
  Failed --> Pending
  Scraping --> Interrupted["interrupted: stale work"]
  Interrupted --> Pending
```
...
```mermaid
flowchart TD
  Open["Open page with an authorized session"] --> Detect{"Protection screen present?"}
  Detect -->|No| Continue["Continue normal data flow"]
  Detect -->|Yes| Wait["Wait and retry within a limit"]
  Wait --> Detect
  Wait -->|"Retry limit reached"| Record["Record failure and stop"]
```
...
```mermaid
flowchart LR
  Browser["Response from the browser"] --> Raw["Raw file archive"]
  Browser --> Normalize["Normalizer"]
  Normalize --> Summary["Main record and summary"]
  Normalize --> Series["Time-series table"]
  Normalize --> Events["Event table"]
  Summary --> Export["Analysis outputs"]
  Series --> Export
  Events --> Export
  Raw -.->|"re-parse"| Normalize
```
...
```mermaid
pie showData
  title Approximate distribution of the 335 MiB working set
  "Raw files" : 146
  "Local database" : 184
  "Other working outputs" : 5
```
